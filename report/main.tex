...
\newcommand{\rc}[1]{\path{#1}}             % ruta o nombre de archivo que puede partirse
\newcolumntype{R}[1]{>{\raggedright\arraybackslash}p{#1}}
\newcommand{\filasep}{\addlinespace[1pt]\midrule\addlinespace[1pt]}  % separa filas de un cuadro

\graphicspath{{figures/}}
\setlength{\parskip}{0.6em}                 % parrafos en bloque (sin sangria)
\setlength{\parindent}{0pt}
\linespread{1.08}
\newcommand{\FloatBarrier}{\par\bigskip}    % sin el paquete placeins
...
resolutivo; a esa poblaci\'on la dej\'e marcada como ``sin ruta''. La mediana del tiempo de acceso es
...
La pregunta que quise responder es : \textbf{?`cu\'anto tarda de verdad la gente en llegar por
...
Todo el proyecto son m\'odulos de Python reutilizables. La Tabla~\ref{tab:archivos} indica qu\'e archivo hace
cada fase.
...
\label{tab:archivos}
\begin{tabular}{@{}R{3.4cm}R{3.3cm}R{7.1cm}@{}}
\toprule
...
\filasep
1 -- adquisici\'on & \rc{src/acquisition.py} & Resuelve cada fuente (usa la copia local, o la copia de \rc{_data/}, o la descarga) y controla el encoding al leer. Registra todo en \rc{data/raw/download_log.json}. \\
\filasep
1 -- validaci\'on & \rc{src/validation.py} & Limpieza, reglas de calidad, recorte a los tres departamentos y guardado en GeoPackage. Escribe \rc{logs/quality_report.csv}. \\
\filasep
1 -- mapas & \rc{src/mapas.py} & Un mapa interactivo por departamento (\rc{data/outputs/mapa_acceso_*.html}). \\
\filasep
2 -- ruteo & \rc{src/routing.py} & Construye el grafo vial desde el \texttt{.pbf} local, engancha los puntos, calcula la matriz de tiempos y la cachea en Parquet. \\
\filasep
3 -- poblaci\'on & \rc{src/poblacion.py} & Estima la poblaci\'on de cada centro poblado a partir de WorldPop. \\
\filasep
3 -- m\'etricas & \rc{src/metrics.py} & Cada indicador es una funci\'on que recibe una tabla y devuelve una tabla. Exporta a \rc{data/outputs/*.csv}. \\
\filasep
4 -- tablero & \texttt{app.py} & Tablero Streamlit; lee solo archivos ya calculados. \\
\filasep
5 -- informe & \rc{src/export.py}, \rc{report/main.tex} & Genera las figuras y tablas del informe y este documento. \\
\bottomrule
\end{tabular}
\end{table}
...
\label{tab:fuentes}
\begin{tabular}{@{}R{2.5cm}R{2.9cm}R{2.7cm}R{5.7cm}@{}}
\toprule
...
Las categor\'ias vienen sin estandarizar; cerca del 37\,\% de los registros
no trae coordenada; que figure como ``activo'' no garantiza que tenga
personal ni insumos. El registro cambia mes a mes: en la versi\'on de agosto
de 2026 el establecimiento 00003598 de Huamanga ya aparece como Hospital
San Juan Bautista \mbox{II-1} (en abril era Centro de Salud \mbox{I-4}). \\
\filasep
...
Cobertura dispareja: buena en ciudades, escasa en zona rural y amaz\'onica. \\
\filasep
...
p\'ublico estuvo inalcanzable los d\'ias de trabajo. La soluci\'on
final ---leer el \texttt{.pbf} local.
...
\figsihay[0.85\linewidth]{cobertura_bandas}{Porcentaje de poblaci\'on por
...
\figsihay[0.7\linewidth]{ecdf_urbano_rural}{Porcentaje acumulado de
poblaci\'on seg\'un el tiempo de acceso, separando urbano y rural. La l\'inea
punteada marca el umbral de \umbralOro{} minutos.}
...
de altitud. La correlaci\'on de Spearman entre tiempo de acceso y altitud es
\spearmanAltitud.}
...
\figsihay[0.55\linewidth]{recta_vs_red}{Distancia por carretera frente a
...
\item \textbf{La red de OpenStreetMap es dispareja.} En zonas rurales y
...
\item OpenStreetMap contributors / Geofabrik. Extracto de Per\'u.\\
...
Project, Per\'u, ajustado a totales ONU. DOI:10.5258/SOTON/WP00685.
