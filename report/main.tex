...
Le\'i la red vial directamente del archivo local de OpenStreetMap
(\texttt{peru-latest.osm.pbf}) con \texttt{pyosmium} y la arm\'e como un grafo
de \texttt{NetworkX}: cada cruce es un nodo y cada tramo de calle, una arista
con su longitud. Sobre ese grafo calcul\'e, para cada centro poblado, el
tiempo al establecimiento resolutivo m\'as cercano.
...
un dato sobre el estado de las herramientas de datos abiertos. Leer el
\texttt{.pbf} local con \texttt{pyosmium} no depende de internet ni de
permisos de administrador, y es completamente reproducible.

\textbf{Decisi\'on 4: el grafo es de \texttt{NetworkX}, pero el camino m\'as
corto lo resuelve \rc{scipy.sparse.csgraph}}, no la funci\'on de Dijkstra
de \texttt{NetworkX}. Es el mismo algoritmo, sobre el mismo grafo y con los
mismos pesos ---el resultado es id\'entico---, solo que \texttt{SciPy} opera
de forma vectorizada sobre la matriz de adyacencia y resuelve de una sola vez
todos los or\'igenes contra todos los destinos. Con la implementaci\'on de
\texttt{NetworkX}, solo la matriz de Piura tardaba unos 47 minutos; con
\texttt{SciPy}, unos 2 segundos. Por eso el grafo, los pesos y el enganche de
los puntos a la red son de \texttt{NetworkX}, y solo el solver final es de
\texttt{SciPy}.
